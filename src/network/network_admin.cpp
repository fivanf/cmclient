...
NetworkRecvStatus ServerNetworkAdminSocketHandler::Receive_ADMIN_RPC_REQUEST(Packet &p)
{
	...
	int64_t arg2 = static_cast<int64_t>(p.Recv_uint64());
	...
	switch (command) {
		case 0: {  // set_company_max_loan
			...
			fmt::println("Setting max loan {} for company {}", (uint64_t)max_loan, arg1);
			auto res = citymania::cmd::SetCompanyMaxLoan(c->index, max_loan)
